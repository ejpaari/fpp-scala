...
object Huffman {
  ...
  /**
    ...
    *   List(('a', 2), ('b', 1))
    */
  def times(chars: List[Char]): List[(Char, Int)] = timesCounter(List(), chars, chars.toSet)
  ...
  def insert(tree: CodeTree, trees: List[CodeTree]): List[CodeTree] =
    trees match {
      case List() => List(tree)
      case y :: ys =>
        if (weight(tree) <= weight(y)) tree :: trees
  ...
  def decodeIn(tree: CodeTree, abs: CodeTree, bits: List[Bit]): List[Char] =
    tree match {
      case Leaf(c, _) => c :: decode(abs, bits)
      case Fork(l, r, _, _) =>
        if (bits.isEmpty) List()
        else decodeIn((if(bits.head == 0) l else r), abs, bits.tail)
    }
  ...
  /**
    ...
    */
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = text.flatMap(e => singleEncode(tree, e))

  def singleEncode(tree: CodeTree, char: Char): List[Bit] =
    tree match {
      case Leaf(_,_) => Nil
      case Fork(l,r,_,_) =>
        if (chars(l).contains(char)) 0 :: singleEncode(l, char)
        else 1 :: singleEncode(r, char)
    }
  ...
  /**
    ...
    */
  def convert(tree: CodeTree): CodeTable = chars(tree).map(e => (e, singleEncode(tree,e)))
